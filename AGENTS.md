...
- `src/` contains the runtime and server implementation (`main.zig`, `server.zig`, `runtime.zig`, `bindings.zig`, `jsx.zig`).
- `zquickjs/` is the pure-Zig JavaScript engine (parser, VM, GC, value system, etc.).
...
- `zig build test-zquickjs` — unit tests for the JS engine.
...
- Files are short, descriptive, and lowercase (e.g., `runtime.zig`, `server.zig`).
...
- Add tests near the feature you touched in `src/` or `zquickjs/` and run the relevant `zig build test*` step.
