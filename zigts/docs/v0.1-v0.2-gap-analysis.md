...
This document maps every section of the v0.2 spec against the current v0.1 implementation in the `zigts/` compiler/engine layer only. Server-layer concerns (`src/server.zig`, `src/zruntime.zig`, `src/main.zig`) are out of scope.
...
|-----------|-------------|
| `*.zigts.ts` canonical | Not recognized. v0.1 uses `.js`, `.jsx`, `.ts`, `.tsx` |
| `*.zts` optional | Not recognized |

**Gap:** New extension recognition needed in the compiler entry points. The `.zigts.ts` extension would also need TS stripping + ZTS-specific passes.
...
| v0.2 Spec | v0.1 Status |
|-----------|-------------|
| `zigts:std` | Does not exist |
| `zigts:rt` | Does not exist |

**Gap:** Entirely new. Requires a virtual module resolver that intercepts `zigts:*` specifiers and returns pre-defined type/value exports. `zigts:std` surfaces the Effect namespace, Err namespace, capability interfaces, and standard capabilities. `zigts:rt` provides the runtime harness with `run`.
...
### 2.2 No hidden magic / `zigts explain`

| v0.2 Spec | v0.1 Status |
|-----------|-------------|
| `zigts explain` command | Does not exist. No desugar tracking infrastructure in the compiler. |
...
| `zigts:rt` runtime harness | Does not exist |
...
**Gap:** The `Result.ok/err/match` builtin in v0.1 is related but different. v0.2's `Err.*` namespace provides error definition, construction, narrowing, and matching. This needs to be implemented in the `zigts:std` virtual module and enforced by the validator.
...
| v0.2 Spec | v0.1 Status |
|-----------|-------------|
| `zigts build` CLI | CLI is out of scope. The compiler pipeline itself is relevant. |
| Native target | Bytecode + JIT produces native execution |
| WASM/WASI target (default) | No WASM codegen backend in the compiler |
| `zigts eject` to TS | No AST-to-source emitter exists |
...
| Virtual modules (`zigts:std`, `zigts:rt`) | Medium | New resolver + predefined exports |
...
| File extensions (`.zigts.ts`, `.zts`) | Small | Extension recognition in compiler entry points |
