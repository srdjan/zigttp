...
pub const binding = mb.ModuleBinding{
    ...
    .name = "http",
    .exports = &.{
        .{ .name = "parseCookies", .func = parseCookiesNative, .arg_count = 1,
           .returns = .object, .param_types = &.{.string},
           .effect = .none, .return_labels = .{ .user_input = true, .credential = true } },
        .{ .name = "setCookie", .func = setCookieNative, .arg_count = 3,
           .returns = .string, .param_types = &.{ .string, .string, .object },
           .effect = .none, .return_labels = .{ .internal = true },
           .contract_extractions = &.{.{ .arg_position = 0, .category = .cookie_name }} },
        .{ .name = "negotiate", .func = negotiateNative, .arg_count = 2,
           .returns = .optional_string, .param_types = &.{ .string, .string },
           .effect = .none, .failure_severity = .expected },
        .{ .name = "parseContentType", .func = parseContentTypeNative, .arg_count = 1,
           .returns = .object, .param_types = &.{.string} },
        .{ .name = "cors", .func = corsNative, .arg_count = 2,
           .returns = .object, .param_types = &.{ .string, .object },
           .effect = .none,
           .contract_extractions = &.{.{ .arg_position = 0, .category = .cors_origin }} },
    },
};
