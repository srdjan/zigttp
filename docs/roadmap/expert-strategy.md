...
| ZTS620 | Boolean compared to a boolean literal       | Use the boolean directly (`x` / `!x`)       | One canonical boolean-test shape; flow checker skips literal-comparison nodes |

ZTS612 through ZTS618 are the most recent slice (commit `c0b66fa`, May 2026). They round out the expression-level alternates. ZTS619 picks up the first item from the loop-shape audit: a `for (const pair of arr.entries())` whose body destructures `[_i, item]` and never reads `_i` collapses to `for (const item of arr)`, dropping a binding the analyzer would otherwise have to track only to prove dead. ZTS620 closes the boolean-test equivalence class: for a statically-boolean `x`, `x === true` is exactly `x` and `x === false` is exactly `!x`, so the literal comparison is a redundant spelling of the test. The rule fires only when the type checker proves the compared operand is boolean. For a non-boolean `x`, `x === true` is an identity test that differs from the truthiness test `x`, so the guard is load-bearing for soundness.
...
- **Continue the equivalence-class audit.** ZTS612-618 closed the expression-level alternates the original audit identified. ZTS619 closed the first loop-shape alternate (unused index alias in `for...of`). ZTS620 closed the boolean-test alternate (comparing a boolean against a boolean literal). The two candidates this doc previously named were audited and **rejected** as failing the "earn its cost" bar:
  - *Redundant `as const` on already-immutable literals*: moot. The stripper rejects **all** `as` assertions wholesale (`UnsupportedAsAssertion`, feature `as_assertion`); `as const` never reaches the analyzer, so there is no legal form to flag as redundant. The failure class is already eliminated by the blanket `as` ban.
  - *Parameter destructure on call sites where the callee declares the wider shape*: unsound. Rewriting `f({a: src.a, b: src.b})` to `f(src)` changes observable behavior whenever `src` carries excess properties (visible via `Object.keys`/`JSON.stringify`, both supported in ZigTS). A canonical rule must preserve semantics, so this rewrite cannot be one.
    ZTS620 was adopted in their place as a sound, type-guarded member of the same audit. Each future candidate still needs a write-up that names the proof it unlocks; don't add rules whose only justification is consistency.
- **Cassette harness for provider tests.** *Landed.* The record/replay harness is at `packages/pi/src/providers/cassette_client.zig` (replay) and `cassette_record.zig` (record), driven by `ZIGTTP_CASSETTE_MODE` (`replay` default / `record` / `passthrough`). Cassettes are JSONL (header + body, with SSE sidecars for streaming) and round-trip both the Anthropic and OpenAI wire formats through the live SSE parsers. Offline test target: `zig build test-cassette`.
- **Streaming for OpenAI.** *Landed (Slice E).* The OpenAI client streams the Responses API (`packages/pi/src/providers/openai/`), assembling tool calls in real time exactly as the Anthropic path does; the loop is provider-agnostic over the assembled `ModelCallResult`.

### Mid-term: structural tool evolution

- **AST-aware tool surface.** The `zigts_expert_ast_rewrite` tool dispatches a typed `RepairIntent` into a verified in-memory rewrite (parse, transform, `edit_simulate` veto) without touching disk. Shipped intents, all line-local canonicalize refactors driven by the diagnostic the compiler already emits: `replace_let_with_const`, `canonicalize_for_of_const`, `replace_arrow_with_function`, `replace_export_arrow_with_function`, `replace_compound_assign_with_explicit` (ZTS613, `x += e` -> `x = x + e`; conservative, refusing call/index targets that would re-evaluate a side-effecting base), and the file-scoped `canonicalize_capability_key_alias`. The transforms live in `packages/tools/src/canonicalize.zig`; the dispatch in `packages/pi/src/tools/repair_apply.zig` and `zigts_expert_ast_rewrite.zig`.
  
  The grander primitives the original audit named were assessed against this line-local, diagnostic-driven infrastructure and **deferred**, because each needs analysis the canonicalize path does not carry and would be its own initiative:
  
  - `narrow_type` is not mechanically determinable: ZTS600 (`implicit_unknown`) fires precisely where inference *failed*, so there is usually no concrete type to insert; its own help text offers three different remedies (annotate, use a modeled module, or add a type guard).
  - `inline_const` needs single-use and evaluation-order analysis the path lacks, and it pulls *against* the profile's explicit name-preference (ZTS615 hoists expressions into named `const`s; ZTS608 prefers named functions). Removing names is rarely the canonical direction here.
  - `rename_symbol`, `extract_function`, `lift_effect` are multi-site/structural transforms that belong with the system-wide-reasoning work below, not the per-handler line-local dispatch.
  
  The realized direction is therefore "make every mechanical, sound, line-local `RepairIntent` dispatchable through the tool," closing the loop the structured-repair-intents item describes: the compiler emits a typed intent, and the agent applies it without inferring the transform from prose.

- **Persistent witness corpus and few-shot context.** *Landed.* `pi_witnesses` persists falsifying inputs under `.zigttp/witnesses/`, and `expert_persona.zig` injects a curated subset into the system prompt under a "WITNESSED FAILURES" section (selected by `witness_corpus.selectFewShot` within an 8 KiB budget). The model sees "this input class breaks handlers like this one" and is biased away from it; the real veto still runs, so the change is soundness-neutral.

- **Learned veto-prediction.** *Not started; blocked on a trace substrate.* A small classifier trained on the tool-call trace (features: rule codes that fired in recent simulations, edit shapes, file regions touched) could predict which proposed edits will fail veto, letting the agent short-circuit obvious rejections without a model turn. The prerequisite is missing: there is no persisted per-call trace recording which rule codes fired against which edit shapes; `edit_simulate` returns counts, not a rule-by-rule firing log. Building that trace-logging substrate is the first step; the classifier is downstream of it. Soundness-neutral either way: the real veto still runs on whatever the predictor lets through.

- **Cross-session memory.** *Landed.* A project-scoped `.zigttp/memory.jsonl` (`packages/pi/src/memory_store.zig`) holds durable facts; the `pi_remember_fact` tool appends and `pi_recall_facts` reads, and `expert_persona.zig` injects a budgeted "PROJECT MEMORY" section (pinned entries first, then most-recent) at session start. This is distinct from the per-session AGENTS.md/CLAUDE.md project-context walk: memory survives across sessions.
...
- **Structured repair intents in every diagnostic.** *Landed (the enum and its emission).* The typed `RepairIntent` enum lives in `packages/zigts/src/repair_intent.zig` and every veto-able diagnostic carries one: a ZTS612 emits `replace_ternary_with_if`, a ZTS617 emits `lift_default_to_body`, a ZTS613 emits `replace_compound_assign_with_explicit`, and so on through the ZTS6xx/ZTS3xx/ZTS4xx families. It is serialized for the model via `zigts describe-rule --json`. *Remaining:* not every emitted intent has a matching mechanical apply primitive yet. `zigts_expert_ast_rewrite` dispatches the line-local canonicalize intents (see the mid-term AST item), while statement-restructuring intents like `replace_ternary_with_if` and `lift_default_to_body` still fall back to a model-authored edit through the veto. Closing that gap is incremental: each new sound, mechanical transform added to the dispatch makes one more intent directly applicable.
- **Performance as proof for the agent itself.** *Landed (apply-time wiring).* Every edit the expert applies now fires a best-effort perf probe over the post-apply handler and appends a signed `kind=perf` row to `.zigttp/proofs.jsonl`: an Ed25519-signed JWS (same persistent attest key as deploy receipts) committing to the handler hash, p50/p99 latency, allocator bytes, and sample count. The wiring is a cycle-free port (`packages/pi/src/perf_probe.zig`) the runtime fills with the engine-backed probe at startup (`packages/runtime/src/perf_probe_lib.zig`, injected in `dev_cli.zig` next to witness-replay); the probe needs the JS engine and the ledger pulls in the deploy stack, neither importable from PI directly. Default on; opt out with `--no-perf-receipt`. A handler that faults on probe (or has no measurable samples) records a `sample_count == 0` row so an audit can prove no receipt was suppressed. *Remaining:* a regression budget (`baselineRef`/`regressionBudgetUs`, slot-reserved in `perf_receipt.zig`) that compares each receipt against the prior one and gates on >N% drift; surfacing the receipt in the live HUD `VerifiedPatch` card; and drawing the probe corpus from real requests rather than a single `undefined`-argument call.

- **Proof-carrying changes (equivalence receipts).** *Landed.* Alongside the perf receipt, every applied edit emits a signed `kind=equivalence` row: the behavioral verdict between the pre- and post-edit handler. The verdict is the classification `contract_diff` already computes (`equivalent`, `equivalent_modulo_laws`, `additive`, `breaking`), with one behavioral upgrade: a changed or removed response path reads as `breaking` even when the route surface is unchanged, which a surface diff alone misses. The receipt commits to the before/after contract hashes, the per-path behavior delta, the laws the equivalence proof relied on, and a `claim_scope` (`pure`/`deterministic`/`structural`) that keeps an effectful handler from ever reading as a clean behavioral equivalence. It is signed with the persistent attest key (`packages/zigts/src/equivalence_receipt.zig`) and appended to `.zigttp/proofs.jsonl`. Same cycle-free seam as the perf probe: `packages/pi/src/equivalence_probe.zig` filled at startup by `packages/runtime/src/equivalence_probe_lib.zig`. The verdict computation is shared with the `zigttp prove-behavior <before.ts> <after.ts>` machine command (a one-shot CI gate, exit 1 on breaking). Default on; opt out with `--no-equivalence-receipt`. This is the artifact behind the flagship claim: the compiler proved the AI's change did not alter your handler's behavior, and signed the proof. *Remaining:* dynamic corroboration that runs both versions over the `gen-tests` path corpus through the replay harness to confirm equivalence up to depth K and synthesize the exact distinguishing request on a `breaking` verdict.
