...
The v0.1.0 hardening pass added FailingAllocator walkers for `Runtime.create` (`packages/zigts/src/pool.zig` test "Runtime.create errdefer ladder closes every failure path"), `parseContractJson` (`packages/runtime/src/contract_runtime.zig`), and the precompile build path. `HandlerPool.initWithEmbeddedAndDeps` (`packages/runtime/src/runtime_pool.zig:122`) is the next natural target: it allocates a `LockFreePool`, a `BytecodeCache`, an optional trace file, and a `TraceMutex`, then calls `prewarm()`. The current `errdefer self.deinit()` at `:180` lands *after* the trace-file + mutex setup, so an allocation failure between `openTraceFile` and `allocator.create(TraceMutex)` could leak the file descriptor without surfacing in a regular test run.

The reason this is deferred with the zruntime split rather than added in isolation: a useful FailingAllocator walker for `HandlerPool.init` has to supply real handler bytecode for `prewarm()` to compile, which means setting up a minimal-but-valid JS handler harness in the test. That harness already exists in the 47-test scaffold inside zruntime.zig; once the test scaffolding moves into `packages/runtime/src/testing/` per step 2 of the v0.2.0 split, the same harness becomes reusable from `runtime_pool.zig` tests. Adding the test before the move would duplicate the harness.
