...
Both binaries share the engine (`zigts`), core server/runtime code (`server.zig`, `zruntime.zig`, `runtime_pool.zig`, `contract_runtime.zig`), `self_extract.zig`, and `cli_shared.zig` (arg parsing, watch sets, size helpers). Build-time `runtime_feature_options` select the optional developer surfaces: `zigttp` links proven live reload, Studio, local deploy, proof-review data, and the `pi_app` agent for the primary `zigttp expert` flow, while `zigttp-runtime` links stubs and rejects `--watch` / `--studio` with a direct error. The standalone `zigts` binary also links `pi_app` for the secondary `zigts expert` flow.
