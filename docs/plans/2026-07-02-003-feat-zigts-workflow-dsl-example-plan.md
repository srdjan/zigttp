...
type: feat
topic: zigts-workflow-dsl-example
...
Zigttp now has durable runs, durable steps, queued workflow dispatch, proof diagnostics, dead letters, and replay paths, but the learning path is still split across primitive-focused examples and docs.
...
- **Queue vocabulary stays split:** `--workflow-queue` is persisted child dispatch for workflows, `zigttp:queue` is process-local actor mailbox messaging, and `fanout` should be described as ordered durable batch grouping rather than real concurrency.
...
- A3. Zigttp maintainer: keeps the example, docs, and verification gates aligned with shipped workflow semantics.
...
- R11. The queue-disambiguation note must separate persisted workflow queue behavior from the `zigttp:queue` actor mailbox surface.
...
- `docs/ideation/2026-07-02-zigts-embedded-workflow-dsl-ideation.html`
...
- `zigttp check HANDLER --json` emits the proof receipt, and
  `zigttp check HANDLER --contract` writes `contract.json` for examples that
...
**Verification:** The fixture passes through `scripts/test-examples.sh` after U2 adds harness coverage, `zigttp check FIXTURE --json` emits proof-trace fields for docs to describe, and `zigttp check FIXTURE --contract` writes durable workflow contract fields.
...
Keep the queue vocabulary rail near workflow-queue docs: `--workflow-queue` persists child dispatch, `zigttp:queue` is actor mailbox messaging, and `fanout` is ordered durable batch grouping.
...
- `fanout` wording avoids real-concurrency claims and points to `zigttp:io.parallel` for true concurrent I/O.
...
| `zig build test-zigts` | U4 | Analyzer and ZTS509 coverage remains green. |
