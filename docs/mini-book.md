...
Each virtual module export carries metadata that the compiler uses for verification, contract extraction, and effect classification. Module bindings also declare what runtime capabilities their Zig implementation consumes (clock, crypto, stderr, etc.); checked helpers enforce these at call time without affecting handler contracts or sandbox policy. The compiler reads this metadata to prove properties about your handler.
