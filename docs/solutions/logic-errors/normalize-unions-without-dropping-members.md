...
(`packages/zigts/src/type_checker.zig:1349`). When `A` was already
...
`string | number | boolean` (`packages/zigts/src/type_pool.zig:1300`).
...
index unchanged (`packages/zigts/src/type_pool.zig:374`).
...
regression now lives at `packages/zigts/src/type_checker.zig:2404`.
...
(`packages/zigts/src/type_env.zig:1169`).
...
- Treating the 16-entry deduplication scratch array as a maximum would be
  ...
  (`packages/zigts/src/type_pool.zig:1166`); dropping the seventeenth member can
...
sufficient (`packages/zigts/src/type_pool.zig:329`). Exact index equality is
...
(`packages/zigts/src/type_pool.zig:382`). This collapses `string | string` to
the primitive `string` index (`packages/zigts/src/type_pool.zig:2123`).
...
(`packages/zigts/src/type_pool.zig:396`). Overflow therefore degrades only
...
overflow (`packages/zigts/src/type_pool.zig:385` and
`packages/zigts/src/type_pool.zig:400`). A two-item top-level input can expand
...
member changes (`packages/zigts/src/type_env.zig:549`). Copying first also keeps
...
(`packages/zigts/src/type_env.zig:552`).

## Why This Works

The nested and direct forms now expose the same ordered flat member slice and
are mutually assignable (`packages/zigts/src/type_pool.zig:2109`). TypePool does
not intern equivalent multi-member nodes: each construction may still append a
new node (`packages/zigts/src/type_pool.zig:385`). Its canonicalization contract
...
(`packages/zigts/src/type_pool.zig:376`). For multi-input unions, every leaf
increments `flattened_count` even after dedup overflow, and the fallback appends
every non-union leaf (`packages/zigts/src/type_pool.zig:347` and
`packages/zigts/src/type_pool.zig:360`). This is the same polarity discipline as
the sibling intersection fallback, which retains raw constraints rather than
silently dropping them (`packages/zigts/src/type_pool.zig:419`).
...
- Test canonical shape and semantic equivalence separately. Compare flattened
  ...
  interning (`packages/zigts/src/type_pool.zig:2109`).
- Preserve constructor identity contracts with a dedicated sole-union-member
  regression (`packages/zigts/src/type_pool.zig:2098`).
- Exercise bounded normalization one member past its scratch limit and assert
  that every input obligation survives (`packages/zigts/src/type_pool.zig:2132`).
- Check capacity after recursive expansion, including a small top-level input
  ...
  (`packages/zigts/src/type_pool.zig:2148`).
- Audit downstream member-copy buffers whenever a shared constructor starts
  ...
  and a generic beyond the former boundary (`packages/zigts/src/type_env.zig:1194`).
- Keep the inference-level annotation regression alongside pool-level tests so
  ...
  (`packages/zigts/src/type_checker.zig:2404`).

## Related Issues

`addIntersection` documents the sibling lossless-overflow rule at
`packages/zigts/src/type_pool.zig:419`. No existing `docs/solutions/` entry or
