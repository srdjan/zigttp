...
|--------|--------|------------|---------------|
| Dev modifies hook scripts | Attacker with repo write access changes `.claude/settings.json` hooks to suppress violation reports or exfiltrate data | Hooks are advisory-only (PostToolUse cannot block edits); `settings.json` is version-controlled; code review catches changes | If merged uncaught, hook could suppress violation reports silently |
