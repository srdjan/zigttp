...
- Source-mapped runtime traps: the JS engine now carries a per-function line table (source `offset -> line:column`) from IR through bytecode generation, peephole compaction, and the bytecode cache (format v4), so an interpreter-tier type fault records the original source line of the faulting instruction (`Interpreter.last_error_location`). Foundation for reporting `.ts` line:column on a runtime fault. JIT-tier source maps and filename threading are follow-ups.
