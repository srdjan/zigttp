...
- v0.1.0-beta polish slice: capability-denial unit tests in `packages/zigts/src/module_binding.zig` pin both halves of policy gating (the `ctx.capability_policy.allowsXxx` check and the active-module `.policy_check` declaration) across env, cache, sql, sql-write, and egress; deterministic HTTP-parser fuzz harness in `packages/runtime/src/http_parser.zig` covers parseRequestLine, parseQueryString, parseContentLengthValue, parseContentLength, and findHeaderEnd across 16k+ seeded iterations and pins regression behavior for CR/LF preservation and `..` path traversal preservation; studio JSON/ndjson responses are bounded at 1 MiB via the new `finishStudioOwnedResponse` helper, returning 413 on overflow instead of unbounded `setBodyOwned`. Slice also surfaces a deferred leak: `Runtime.installHttpConstructors` (and structurally similar `install*` siblings in `zruntime.zig`) orphan the just-created prototype JSObject when a midway `addDynamicMethod` returns `OutOfMemory`, because the prototype is appended to `ctx.builtin_objects` only after methods are added. Caught by the wired-but-`SkipZigTest`'d `HandlerPool init under FailingAllocator never leaks`; structural fix slated for v0.2.0.
...
- `docs/internals/feature-matrix.md`: cross-reference between the documented JS/TS subset and the engine layers that implement each feature (parser, IR, codegen, interp, baseline JIT, optimized JIT). Identifies 17 features documented as supported but lacking handler-level test fixtures, plus a recipe for finishing the JIT-tier walk.
- `docs/releases/v0.1.0-scope.md`: one-page "what's in / what's deferred" for the first tag, linked from README. Names hosted cloud deploy, the evented I/O backend, runtime multipart parsing, and JIT code-cache eviction as explicit non-goals for v0.1.0.
...
- `docs/roadmap/cli-tools-extraction.md`: roadmap doc for the deferred v0.2.0 extraction of `proof_card_tui.zig`, `proof_ledger.zig`, `proof_quest.zig` (proof_audit_ring stays — it's on the server hot path). Documents why the extraction needs to ride the deploy/ refactor.
