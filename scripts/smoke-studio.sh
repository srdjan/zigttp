#!/bin/bash
# Smoke test for the opt-in browser proof workbench (zigttp studio).
#
...
#   ZIGTTP=/path/to/zigttp  to skip the build and use an existing -Dstudio binary.
...
TMP_DIR=$(mktemp -d -t zigttp-smoke-studio-XXXXXX)
...
step "build zigttp with -Dstudio"
