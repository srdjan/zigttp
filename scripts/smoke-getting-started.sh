...
TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/zttp-getting-started.XXXXXX")
APP_NAME="my-app"
APP_DIR="$TMP_DIR/$APP_NAME"
HTMX_DIR="$TMP_DIR/htmx-app"

cleanup() {
    pkill -f "zttp.*--port $DEV_PORT" 2>/dev/null || true
...
[ -x "$REPO_ROOT/zig-out/bin/zttp-runtime" ] || fail "missing zttp-runtime after build"

step "default help lists only the five core commands"
help_out="$("$ZTTP" --help)"
for verb in init dev test expert deploy; do
    printf '%s' "$help_out" | grep -q "zttp $verb" || fail "core command missing from help: $verb"
done
for hidden in check serve compile proofs; do
    printf '%s' "$help_out" | grep -q "zttp $hidden " && fail "advanced command leaked into default help: $hidden"
done

step "init api project"
(cd "$TMP_DIR" && "$ZTTP" init "$APP_NAME" --template api >/dev/null) || fail "init api"
[ -f "$APP_DIR/zttp.json" ] || fail "missing zttp.json"
...
"$ZTTP" dev --port "$DEV_PORT" --no-tour >/tmp/zttp-getting-started-dev.log 2>&1 &
DEV_PID=$!
health_body=$(wait_for_http "http://127.0.0.1:$DEV_PORT/health" 75) || {
    cat /tmp/zttp-getting-started-dev.log >&2 || true
    fail "dev did not serve /health"
}
printf '%s' "$health_body" | grep -q '"ok":true' || fail "unexpected /health body: $health_body"
stop_bg "$DEV_PID" "zttp.*--port $DEV_PORT"

step "test"
"$ZTTP" test >/dev/null || fail "test"

step "deploy and run artifact"
"$ZTTP" deploy >/dev/null 2>&1 || fail "deploy"
[ -x "$APP_DIR/.zttp/deploy/$APP_NAME" ] || fail "missing deploy artifact"
[ -s "$APP_DIR/.zttp/proofs.jsonl" ] || fail "missing proof ledger"
...
(cd "$TMP_DIR" && "$ZTTP" init htmx-app --template htmx >/dev/null) || fail "init htmx"
...
(cd "$HTMX_DIR" && "$ZTTP" test >/dev/null) || fail "htmx test"
