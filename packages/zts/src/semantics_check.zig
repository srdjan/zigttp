...
const jws_header = "{\"alg\":\"EdDSA\",\"typ\":\"zttp-semantics+jws\"}";
const payload_version = "zttp-semantics-v5";
...
test "receipt verify rejects a valid signature with a non-semantics payload" {
    ...
    const compact = try signRawCompactForTest(allocator, jws_header, "zttp-build-v1\nok", kp);
    defer allocator.free(compact);
