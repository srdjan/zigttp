...
pub fn check(
    ...
    if (inputs.imports_websocket_module and !inputs.exports_on_message) {
        try out.append(allocator, .{
            ...
            .severity = .warning,
            .message = "zttp:websocket is imported but no onMessage handler is exported",
    ...
    if (!inputs.imports_websocket_module and inputs.exportsAnyEvent()) {
        try out.append(allocator, .{
            ...
            .severity = .err,
            .message = "WebSocket event handlers are exported but zttp:websocket is not imported",
            .help = "Add `import { send, close } from 'zttp:websocket';` so event handlers can reply to frames.",
        });
    }
}
