...
//!
//! This lives in the zts layer (like `perf_receipt.zig`) so both the tools
...
pub fn sign(
    ...
    const header_json = "{\"alg\":\"EdDSA\",\"typ\":\"zttp-equivalence+jws\"}";
...
test "signing input is stable for identical verdicts" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, a, "zttp-equivalence-v1\n"));
}
