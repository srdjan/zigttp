...
fn runProvePipeline(
    ...
    if (prove_trace_path) |trace_path| {
        ...
        if (groups.len > 0) {
            ...
            const replay_result = runBuildTimeReplay(allocator, source, handler_path, groups) catch |err| {
                std.debug.print("Replay verification failed: {}\n", .{err});
                return err;
            };
            prove_replay = .{
    ...
    const prove_dir = deriveSiblingPath(allocator, output_path, "") catch |err| {
...
fn writeBuildReport(
    ...
    const report_path = deriveSiblingPath(allocator, output_path, "report.json") catch |err| {
