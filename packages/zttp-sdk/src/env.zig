...
extern fn zttpSdkReadEnv(handle: *ModuleHandle, name_ptr: [*]const u8, name_len: usize, out_ptr: *[*]const u8, out_len: *usize) bool;
extern fn zttpSdkAllowsEnv(handle: *ModuleHandle, name_ptr: [*]const u8, name_len: usize) bool;
...
pub fn readEnv(handle: *ModuleHandle, name: []const u8) ?[]const u8 {
    ...
    var len: usize = 0;
    if (!zttpSdkReadEnv(handle, name.ptr, name.len, &ptr, &len)) return null;
    return ptr[0..len];
}

/// Ask the capability policy whether `env(name)` is permitted.
pub fn allowsEnv(handle: *ModuleHandle, name: []const u8) bool {
    return zttpSdkAllowsEnv(handle, name.ptr, name.len);
}
