...
//! happens via the persistent keypair at `~/.zigttp/attest/keypair.bin`
...
pub const version_tag: []const u8 = "zigttp-perf-v1";
pub const jws_typ: []const u8 = "zigttp-perf+jws";
...
test "perf_receipt version tag string is stable across builds" {
    try testing.expectEqualStrings("zigttp-perf-v1", version_tag);
    try testing.expectEqualStrings("zigttp-perf+jws", jws_typ);
}
