...
//! party modules ship their proof metadata as JSON `zigttp-module.json` files
...
test "registry register and lookup round-trip" {
    ...
    const looked_up = registry.fromSpecifier("zigttp-ext:stripe") orelse return error.TestExpectedManifest;
    try std.testing.expectEqualStrings("zigttp-ext:stripe", looked_up.specifier);

    const exp = registry.findExport("zigttp-ext:stripe", "chargeCard") orelse return error.TestExpectedExport;
...
test "registry fromSpecifier returns null on miss" {
    ...
    try std.testing.expect(registry.fromSpecifier("zigttp-ext:nope") == null);
    try std.testing.expect(registry.findExport("zigttp-ext:nope", "x") == null);
}
