...
fn workflowCallCallback(
    ...
    const saved_runtime = current_runtime;
...
fn workflowSagaCallback(runtime_ptr: *anyopaque, ctx: *zq.Context, steps_val: zq.JSValue) anyerror!zq.JSValue {
    const rt: *Runtime = @ptrCast(@alignCast(runtime_ptr));
