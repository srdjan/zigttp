//! zigts_expert_ratchet - report the proven-property set for a handler.
//!
...
const name = "zigts_expert_ratchet";
...
pub const tool: registry_mod.ToolDef = .{
    ...
    \\the Zigttp-Attest JWS), so it is mechanical and attestable. Use this
...
fn execute(
    ...
    const precompile = @import("zigts_cli").precompile;
...
test "tool description names provenSpecs and the JWS link" {
    ...
    try testing.expect(std.mem.indexOf(u8, tool.description, "Zigttp-Attest JWS") != null);
}
