...
>
> **Drift check, run first**: `git diff --name-only a4a731bd -- packages/zigts/src/property_diagnostics.zig packages/tools/src/system_rollout.zig packages/zigts/src/json_utils.zig`
...
`packages/zigts/src/json_utils.zig` is the canonical, shared JSON-string-escaping helper (already reused by `handler_contract.zig`, `security_events.zig`, `deploy_manifest.zig`, `openapi_manifest.zig`, and `manifest_alignment.zig`). It escapes `"`, `\`, `\n`, `\r`, `\t`, and all other C0 control characters (`0x00-0x08`, `0x0B-0x0C`, `0x0E-0x1F`) as `\u00XX`. Two other files independently reimplement the same logic and both are missing the C0-control-character branch:

- `packages/zigts/src/property_diagnostics.zig:372-383`
...
- Canonical implementation: `packages/zigts/src/json_utils.zig:6-26`:
  ```zig
...
| zigts tests (property_diagnostics, json_utils) | `zig build test-zigts` | success |
...
- `packages/zigts/src/property_diagnostics.zig` — delete the local `writeJsonStringContent` (`372-383`), add `const json_utils = @import("json_utils.zig");` near the existing imports (`16-22`), update the one call site (`330`) to `json_utils.writeJsonStringContent(...)`.
- `packages/tools/src/system_rollout.zig` — delete the local `writeJsonString` (`863-876`), add a local alias `const writeJsonString = zigts.json_utils.writeJsonString;` near the existing `handler_contract`/`system_linker` aliases (`system_rollout.zig:4-6`), so the eight existing call sites (`742, 747, 754, 762, 775, 782, 793, 800`) keep working unchanged.

**Out of scope:**

- `packages/zigts/src/json_utils.zig` itself — canonical, not being changed.
...
- Branch: work on `main`.
- Commit style: Conventional Commits, e.g. `fix(zigts,tools): stop two local JSON-escaping copies from silently emitting invalid JSON`.
...
**Verify**: `zig build test-zigts` -> success. Existing tests `property_diagnostics.zig:457, 468, 494, 553` must still pass unmodified.

### Step 2: Fix `system_rollout.zig`

Add the local alias `const writeJsonString = zigts.json_utils.writeJsonString;`, delete the local `fn writeJsonString` at `863-876`. Do not change the eight call sites' text — the alias makes them resolve to the canonical implementation without edits.
...
**Verify**: `zig build test-zigts test-rollout` -> success; both new tests fail against the pre-fix local implementations (temporarily restore the old local functions locally to confirm, then reapply the fix) — do not skip this red-proof.
...
- [ ] `zig build test-zigts test-rollout` exit 0.
