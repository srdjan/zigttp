...
- **Dormant uncollected tool tests (found executing 009) - RESOLVED**: `packages/tools/src/zigts_cli.zig`, `describe_rule.zig`, and `search_rules.zig` had `test {}` blocks that **no test root collected** (those files are only reached through the `zigts_cli` named module, never an `addTest` root - proven via an aggregate canary), and two `describe_rule.zig` tests used `std.io.fixedBufferStream` (removed in Zig 0.16) so they would not even compile. Fix: repaired the two tests to the 0.16 `std.Io.Writer.Allocating` + `.buffered()` idiom; added a `test-zigts-cli` `addTest` root in `build.zig` (rooted at `zigts_cli.zig`, mirroring `canonicalize_tests`) wired into the aggregate `test`; and added a `test { _ = describe_rule; _ = search_rules; }` reference anchor in `zigts_cli.zig` (a plain `@import` does not pull a file's test blocks - the dev_cli.zig pattern / "CLI test-collection closure"). Red-proven: breaking the revived `writeUnknownRuleJson` test made `test-zigts-cli` fail. test-zigts-cli (75 pass) + full scripts/verify.sh green.
