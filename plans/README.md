...
- **Dormant uncollected tool tests (found executing 009)**: `packages/tools/src/zigts_cli.zig`, `describe_rule.zig`, and `search_rules.zig` have `test {}` blocks (e.g. zigts_cli's "missing sql schema json", describe_rule's "writeUnknownRuleJson"/"writeTypeCheckerJson"/"findTypeCheckerRule") that **no test root collects** - those files are only reached through the `zigts_cli` named module, which is never an `addTest` root. Proven via an aggregate canary (`expect(false)` in zigts_cli.zig left `zig build test` green). Worse, two describe_rule.zig tests use `std.io.fixedBufferStream`, removed in Zig 0.16, so they would not even compile if collected. Fix would mirror the `canonicalize_tests` root in `build.zig` (add a `zigts_cli` test root) AND repair the 0.16 API breakage first. Deferred from 009 to keep that plan's diff scoped to flag rejection; worth a dedicated small plan. S effort, LOW risk.
