...
zig build run -- examples/handler/handler-full.tsx # Full routing + JSX demo
zig build run -- examples/handler/handler-full.tsx --test examples/handler/handler.test.jsonl  # Run handler tests
...
**Handler Verification** (`zts/handler_verifier.zig`): The `-Dverify` build option enables compile-time correctness verification. The verifier statically proves: (1) every code path returns a Response, (2) Result values from virtual modules are checked before access, (3) no unreachable code after returns, (4) unused variables detected with scope-aware tracking (warnings, underscore-prefix suppresses), (5) match expressions have default arms, (6) optional values from `env()`, `cacheGet()`, `parseBearer()`, and `routerMatch()` are checked before use, (7) no module-scope variable mutations inside the handler body (cross-request state isolation). Check 6 tracks optional bindings through the control flow tree, recognizing narrowing via truthiness guards (`if (val)`), negated early returns (`if (!val) return`), explicit undefined checks (`val !== undefined`), nullish coalescing (`val ?? default`), and reassignment. Check 7 detects assignments to bindings with scope_id less than the handler's scope, proving that no data leaks between requests. This is possible because zigttp's JS subset eliminates most non-trivial control flow (`break`/`continue` are allowed within `for-of` as forward jumps only) - the IR tree IS the control flow graph. See [docs/verification.md](docs/verification.md).

**Handler Contract Manifest** (`zts/handler_contract.zig`): Contract extraction runs automatically during every precompilation, scanning the handler's IR for virtual module imports, literal env var names, outbound hosts from `fetchSync` URL arguments, cache namespace strings, route patterns, durable usage (keys, step names, timer usage, signal names, producer keys), and rate limiting patterns (guard composition with `cacheIncr`). The `-Dcontract` build option additionally emits `contract.json` (v9) alongside the embedded bytecode. Non-literal arguments set `dynamic: true` as an honest signal that static analysis cannot enumerate all values.

**Handler Effect Classification** (`zts/modules/resolver.zig`, `zts/handler_contract.zig`): Each virtual module export carries an `EffectClass` annotation (read/write/none) defined in `ModuleExport`. During contract extraction, the `ContractBuilder.computeProperties()` method aggregates these effects across all imported functions to derive handler-level `HandlerProperties`: `pure` (no virtual module calls or egress), `read_only` (only read-classified functions), `stateless` (read_only and no cacheGet), `retry_safe` (read_only, or all writes within durable steps), `deterministic` (no Date.now() or Math.random()), `has_egress` (uses fetchSync), `idempotent` (deterministic and retry_safe - safe for at-least-once delivery), `max_io_depth` (maximum I/O calls on any execution path, from PathGenerator), `injection_safe` (no unvalidated user input reaches sensitive sinks, from FlowChecker), `state_isolated` (no module-scope mutations, from handler verifier Check 7). Properties appear in contract.json, the build report (PROVEN/--- labels), deploy manifest tags, OpenAPI specs (x-zigttp-properties extension), and OWASP compliance mapping.

**Data Flow Provenance** (`zts/flow_checker.zig`): The FlowChecker tracks data labels (`secret`, `credential`, `user_input`, `validated`, `config`, `internal`, `external`) through the IR tree using a single-byte LabelSet per binding. Sources are auto-labeled from virtual module return types (env vars get `secret` for sensitive names, auth tokens get `credential`, request body gets `user_input`). Sinks are checked for policy violations: secrets in responses, credentials in logs, unvalidated input in fetchSync bodies, and unvalidated user input in `Response.html()` (XSS prevention). Result.value label narrowing: when a binding is initialized from a validation call (validateJson, validateObject, coerceJson), accessing `.value` on it inherits the `validated` label. The `injection_safe` property is set to false when any unvalidated user input reaches a sensitive sink.
...
**Behavioral Contract** (`zts/handler_contract.zig`, `zts/path_generator.zig`): `-Dcontract` runs PathGenerator and populates the contract's `behaviors` section with every execution path. Each `BehaviorPath` records route method/pattern, branching conditions (`PathCondition` with kind io_ok/io_fail/req_method/req_url), I/O call sequence (`PathIoCall` with module/func), response status, I/O depth, and failure-path flag. `behaviors_exhaustive` is true when PathGenerator stays below the 1024-path cap. Behavioral paths are serialized in contract.json, deserialized by `parseFromJson`, and compared by `contract_diff.diffBehaviors()` during proven evolution. The diff matches paths by (method, pattern, conditions) tuple and classifies each as preserved, response_changed (same conditions, different status), removed, or added.

**Upgrade Verification** (`tools/upgrade_verifier.zig`, `tools/prove_upgrade.zig`): Combines surface diff, behavioral diff, property regression, and coverage gap analysis into a four-value `UpgradeVerdict`: `safe` (identical behavior, no regressions), `safe_with_additions` (new paths, existing preserved), `breaking` (paths removed, responses changed, or critical property lost), `needs_review` (structurally OK but warning-level regressions or significant coverage gaps). Property regression severity: critical (retry_safe, injection_safe, state_isolated, fault_covered, no_secret_leakage, no_credential_leakage), warning (idempotent, deterministic, read_only), info (pure, stateless). `UpgradeManifest` includes surface summary, behavioral summary, property regressions/gains, and coverage gap. Both `zig build prove` and `-Dprove` emit `upgrade-manifest.json` alongside `proof.json`.
...
- `aws`: Generates AWS SAM `template.json` with proven env vars as parameters, routes as HttpApi events, egress hosts as tags, proof level metadata, and conditional VPC SecurityGroup for proven egress hosts. When egress hosts are proven, a SecurityGroup with HTTPS-only egress rules is generated (activated via `VpcSubnetIds` parameter). Output: `src/generated/deploy/template.json` + `src/generated/deploy/deploy-report.txt`.
...
**Type Checking** (`zts/type_map.zig`, `zts/type_pool.zig`, `zts/type_env.zig`, `zts/type_checker.zig`): Full type annotation checking for TypeScript handlers. The stripper extracts a TypeMap of all type annotations (type aliases, interfaces, variable/param/return annotations, generics) as sideband data without changing the IR or parser. The TypePool stores structured types (primitives, records, unions, functions, arrays, tuples, nullable, generics) as a flat indexed array with structural subtyping via `isAssignableTo`. The TypeEnv resolves type aliases, interfaces, and annotations by walking the TypeMap. Generic type aliases (`type Result<T> = { ok: boolean; value: T }`) are stored with their parameter names and uninstantiated body; when a generic application (`Result<string>`) appears in an annotation, the TypeEnv calls `TypePool.instantiate()` to substitute parameters with concrete types, producing a fully resolved record type for structural checking. Up to 8 type parameters per alias. The TypeChecker walks the IR tree checking: variable declaration type mismatches, function argument types, property access on known records, return type mismatches, and discriminated union narrowing in match expressions. When a match discriminant is a union type and arm patterns are literals, the type checker narrows the union to the matching member within each arm body. If-guard narrowing: `if (x)`, `if (!x) return`, and `if (x !== undefined)` narrow nullable bindings to their inner type within the guarded branch. The negated early-return pattern (`if (!x) return`) also narrows `x` after the if-block. Object literal return values are structurally matched against declared interface/type alias return types (nominal check bypassed for record sources). Literal types (`let x = 0`) are widened to base types for `let` bindings so reassignment works without explicit annotations. Virtual module types (`zts/modules/types.zig`) provide full function signatures for all 27 zigttp:* exports. Interface declarations with all-function members are marked nominal to prevent structural forgery of capability objects. Two-arg handler support (`handler(req, caps)`) detects handler arity at runtime for capability injection.
