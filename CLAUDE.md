...
**Handler Contract Manifest** (`zts/handler_contract.zig`): Contract extraction runs automatically during every precompilation, scanning the handler's IR for virtual module imports, literal env var names, outbound hosts from `fetchSync` URL arguments, cache namespace strings, route patterns, and durable usage (keys, step names, timer usage, signal names, producer keys). The `-Dcontract` build option additionally emits `contract.json` (v4) alongside the embedded bytecode. Non-literal arguments set `dynamic: true` as an honest signal that static analysis cannot enumerate all values.
