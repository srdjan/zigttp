...
Each `ModuleExport` carries an `effect: EffectClass` annotation (read/write/none). Read-effect: all env, crypto, router, auth, validate functions, plus `cacheGet`/`cacheStats`, `sql`/`sqlOne`/`sqlMany`. Write-effect: `cacheSet`/`cacheDelete`/`cacheIncr`, `sqlExec`, `parallel`/`race`, and all durable functions. None-effect: `guard` (compile-time only). The `ContractBuilder.computeProperties()` method aggregates these to derive `HandlerProperties` during contract extraction.
